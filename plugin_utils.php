...
class PluginUtils {
	...
	public function _update_option ( $optionID, $data, $network = false ) {
		if ( $network ) {
			$this->log ( sprintf( __( 'updating network option %s', 'PluginUtils' ), $optionID ) );
			update_site_option( $optionID , $data );
		}
		else {
			$this->log ( sprintf( __( 'updating option %s', 'PluginUtils' ), $optionID ) );
	...
	public function _get_option ( $optionID, $network = false ) {
		if ( $network ) {
			$this->log ( sprintf( __( 'getting network option %s', 'PluginUtils' ), $optionID ) );
			$options = get_site_option( $optionID );
		}
		else {
			$this->log ( sprintf( __( 'getting option %s', 'PluginUtils' ), $optionID ) );
	...
	public function _delete_option ( $optionID, $network = false ) {
		if ( $network ) {
			$this->log ( sprintf( __( 'deleting network option %s', 'PluginUtils' ), $optionID ) );
			delete_site_option( $optionID );
		}
		else {
			$this->log ( sprintf( __( 'deleting option %s', 'PluginUtils' ), $optionID ) );
	 ...
	 *
	 */
	public function log ( $identifier = __FILE__ , $message = __LINE__ , $log_level = LOG_WARNING ) {
